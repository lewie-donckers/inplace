...
TEST(function_test, with_pass_through_const_l_ref) {
    const auto input = 0;
    // NOLINTNEXTLINE(bugprone-return-const-ref-from-parameter) for testing purposes only
...
TEST(function_test, size_requirements) {
    ...
    }
    {
        struct test_type {
            ...
            // NOLINTNEXTLINE(cppcoreguidelines-avoid-c-arrays, modernize-avoid-c-arrays) for testing purposes only
            void* val[2]{};
        };
    ...
    }
    {
        ...
        static_assert(!std::is_constructible_v<function<int(int, int)>, test_type>);
        static_assert(!std::is_constructible_v<function<int(int, int), (6 * pointer_size) - 1>, test_type>);
        static_assert(std::is_constructible_v<function<int(int, int), 6 * pointer_size>, test_type>);
    }
}
